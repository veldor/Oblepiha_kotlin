...
class DeleteMessageWorker(context: Context, workerParams: WorkerParameters) :
    Worker(context, workerParams) {
    override fun doWork(): Result {
        val connector = MyConnector()
        val response = connector.requestDeleteNotification(
            inputData.getString(MiscReceiver.MESSAGE_ID)!!.toInt()
        )
        if (response.isEmpty()) {
            ...
            Handler(Looper.getMainLooper()).post {
                Toast.makeText(
                    App.instance,
                    App.instance.getString(R.string.notification_deleted_message),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
